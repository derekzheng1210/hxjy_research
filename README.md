...
- `/bond-detail`：债券详查，按债券代码、简称或发行人搜索，汇总评级曲线两倍标准差位置、
  主体曲线凸点（拟合时剔除本券）、可切换3/6个月的骑乘收益、经纪商历史报价和主体关键日期利差。页面结论完全由
  确定性规则算法生成，不调用大模型。
...
时刻），默认保留最近 10 个有数据的交易日，超出自动清理；最新快照仍以
`data/broker_market/latest_snapshot.json` 单独维护。历史快照为后续历史行情类功能预留数据，
