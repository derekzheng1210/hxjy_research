...
assert(status.text.includes('正在重试：每日数据更新'));
...
assert(!source.includes('absDeltaBp'));
assert(!source.includes('|变动|'));
// 仅看有授信 / 仅看630合规：默认勾选，恢复默认时同样保持勾选
assert(source.includes('id="onlyCredit" type="checkbox" checked'));
assert(source.includes('id="only630Ok" type="checkbox" checked'));
assert(source.includes("onlyCredit&&(b.counterpartyLimit===null||b.counterpartyLimit<=0)"));
assert(source.includes("only630Ok&&b.rating630!=='ok'"));
assert(source.includes("['onlyCredit','only630Ok'].forEach(id=>document.getElementById(id).checked=true)"));
